...
class enemy(ship):
    def __init__(self,name,width,height,x,y,health,orgHealth,speed,projectiles,left,right,up,down):
        super().__init__("Enemy ship",64,64,random.randint(50,600),random.randint(50,600),500000,500000,5,[],[],False,False,True,False,0)
    ...
    def draw(self,window):
        ...
        try:
            if pattern == 1:
                ...
                if self.part2 == 1 and self.runTimer == 0:
                    ...
                    self.myRand = random.randint(2,3)
                    self.randomNum = self.myRand
        ...
        for o in objectList:
            if o != self and self.suicideAttack != 1:
                ...
                if actualDistanceFromTarget <= self.width + 20 and self.runTimer == 0:
                    self.runTimer = 15
                if self.runTimer >= 1:
                    self.runTimer -= 1
                    if self.randomNum == 0:
                        self.randomNum = random.randint(1,3)
                    if o.x > self.x and abs(o.y - self.y) <= abs(o.x - self.x): #RIGHT
                        self.notRight = True
...
class damage():
    def __init__(self,damageCaused,responsibleProjectile,damageOwner,damageOwnerX,yNumber):
        self.y = 0
                    ...
                    try:
                        ...
                        if p.damage > 0:
                            ...
                            for c in str(p.damage):
                                howFarDown = len(o.damages) * 15

                                if len(str(p.damage)) == 2 and noDuplicates == 0:
                                    damageNumber = [damage(int(str(p.damage)[0]),p.owner,o,0,howFarDown),damage(int(str(p.damage)[1]),p.owner,o,1,howFarDown)]
                                    ...
                                    damageNumber = damage(int(c),p.owner,o,0,howFarDown)
                                    o.damages.append(damageNumber)
                                    noDuplicates = 1
    ...
    for ob in objectList:
        for otherOb in objectList:
            if ob.x + ob.width / 2 > otherOb.hitbox[0] and ob.x < otherOb.hitbox[2] + otherOb.x and ob is not otherOb: 
                if ob.y  + ob.height / 2 > otherOb.hitbox[1] and ob.y < otherOb.y + otherOb.hitbox[3] and ob.name != otherOb.name:
