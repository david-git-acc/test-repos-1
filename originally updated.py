...
shields = [pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/stations/shieldUp.png"),pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/stations/shieldRight.png"),pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/stations/shieldDown.png"),pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/stations/shieldLeft.png"),]
...
healthyActivated = pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/healthyActivated.png")
invisActivated = pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/invisActivated.png")
fireActivated = pygame.image.load(r"C:/Users/david/Desktop/Schoolwork/Y12 Web Tech/My own work/python oop pygame/test2/art/fireActivated.png")
...
def drawWindow():
    ...
    for o in objectList:
        ...
        if not isinstance(o,player) and not isinstance(o, spaceStation) and not isinstance(o, shield) and o.runTimer <= 0 and o.health < 0.51 * o.orgHealth:
        ...
        if not isinstance(self,player):
            ...
            for o in objectList:  
                ...
                if o != self and self.suicideAttack != 1: #Suicide charging enemies will ignore this code for obvious reasons
                    ...
                    if actualDistanceFromTarget <= self.width / 2 + o.width / 2 + 35 and self.runTimer == 0 and self.afkTimer != 0 and self.invisible <= 0 and o.invisible <= 0:
...
class friendly(spaceStation):
    def __init__(self,name,width,height,x,y,health,orgHealth,mitigation,projectiles,horProjectiles,healthRecovery,leftShieldHP,northShieldHP,rightShieldHP,southShieldHP,shieldDistance):
        super().__init__("Friendly station",90,119,501,519,1500,1500,0.65,[],[],15,500,500,500,500,15)
    ...
    def draw(self,window):
        ...
        if self.volleyTimer <= 0:
            ...
            for a in range(numberOfHits):
                aMostPowerfulBullet = torpedo(self.x+self.width/2,self.y + self.height/2,self.name,self,potentialTargets[random.randint(0,len(potentialTargets)-1)])
        ...
        if self.beamTimer <= 0:
            potentialTargets = [a for a in objectList if a.name in enemyTeam and a.x + 35 > self.x + self.width/2 and self.x + self.width/2 + 35 > a.x or a.name in enemyTeam and a.y + 35 > self.y + self.height/2 and self.y + self.height/2 + 35 > a.y  ]
...
class enemy(ship):
    ...
    def draw(self,window):
        ...
        for o in objectList:
            allegianceCheck = isinstance(o,player) or isinstance(o,friendly)
            if allegianceCheck:
        ...
        if self.invisCall == 0 and self.fireCall == 0 and self.healthTarget == 0 or self.invisCall == 0 and self.fireCall == 0 and self.health > self.orgHealth * 0.52: 
            ...
            if self.enemyMemoryX == self.target.x and self.enemyMemoryY == self.target.y and self.afkTimer > 0 and not isinstance(self.target, spaceStation):
                self.afkTimer -= 1
...
class friendlyS(shield):
    def __init__(self, name , x , y , orientation, actualWidth,  actualHeight, health,orgHealth,healthRecovery,orgHealthRecovery,owner):
        super().__init__("friendlyShield",x,y,orientation,16,119,100,100,20,20,owner)


    def draw(self,window):
        if self.health > 0 and self.underAttackTime == 0:
            if self.orientation == 0:
                window.blit(shields[0],(self.x,self.y))
            elif self.orientation == 1:
                window.blit(shields[1],(self.x,self.y))
            elif self.orientation == 2:
                window.blit(shields[2],(self.x,self.y))
            else:
                window.blit(shields[3],(self.x,self.y))
...
class torpedo():
    ...
    def draw(self,window):
        ...
        window.blit(torpedoAnimation[self.animCounter], (self.x,self.y))
    ...
    def draw(self,window):
        ...
        if self.y > winHeight or self.y < 0 or self.ownerID not in objectList: #If the projectiles exit the map
            try:
                ...
                self.ownerID.projectiles.remove(self) #The player ship will always be called "play"
                projectileList.remove(self)
            except:
                pass
            #Unexpected errors may occur
...
alliedTeam = ["Player ship" , "Friendly station","friendlyShield"]
                            ...
                            if p.damage > 0 and o.health > 0:
                                ...
                                if o.health > p.damage and criticalStrike == 0:
                                    ...
                                    if o.name in enemyTeam:
                                        score += int(0.43 * (currentHealth - o.health) ** 2)
