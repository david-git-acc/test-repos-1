...
class ship():
    ...
    def basicFunctions(self,window):
        ...
        if isinstance(self, annoraxPlayable):
        ...
        if not isinstance(self,player) and not isinstance(self,annoraxPlayable):
            ...
            for o in [a for a in objectList if not isinstance(a, spaceStation) and not isinstance(a, shield) and a.runTimer <= 0]:  
...
class enemy(ship):
    ...
    def fireGuns(self, target):
        ...
        if self.afkTimer == 0 and self.runTimer == 0 and self.randomNum == 0 and self.attackPatternLeft != 1 and self.attackPatternRight != 1 and self.attackPatternSouth != 1 and self.attackPatternTop != 1:    
            ...
            if target.y + 15 > self.y and target.y - 15 < self.y:
            ...
            #Same as above
            elif target.x + 15 > self.x and target.x - 15 < self.x:
                #AFK player is BELOW
                if target.y > self.y:
                    self.left = False
                    self.right = False
                ...
                if target.x > self.x:# and abs(o.y - self.y) < abs(o.x - self.x): 
                    ...
                    if target.x + 15 > self.x and target.x - 15 < self.x:
                        pass #Same as above
                    else:
...
alliedTeam = ["Player ship" , "Friendly station","friendlyShield", "Player annorax"]
...
while running:
    ...
    for o in objectList:
        if isinstance(o,enemy) or isinstance(o,miner):
            count += 1
        if isinstance(o, player) or isinstance(o,annoraxPlayable):
            count2 += 1
