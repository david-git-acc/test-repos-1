...
class damage():
    ...
    def draw(self,window):
        if self.damageOwnerX == 1:
            window.blit(damageCounterAllied[self.damageCaused], (self.damageOwner.x+8.1 + self.damageOwner.width * 0.85,self.damageOwner.y + self.y - 10 - self.yNumber))
        else:
            window.blit(damageCounterAllied[self.damageCaused], (self.damageOwner.x + self.damageOwner.width * 0.85,self.damageOwner.y + self.y - 10 - self.yNumber))
        for o in objectList:
            if self in o.damages:
                self.y -= 0.5 * len(o.damages)
        if self.y < -10:
                    ...
                    try:
                        ...
                        if p.damage > 0:
                            ...
                            for c in str(p.damage):
                                howFarDown = 15 * len(o.damages) - 15
                                if len(str(p.damage)) == 2 and noDuplicates == 0:
                                    damageNumber = damage(int(str(p.damage)[0]),p.owner,o,0,howFarDown)
                                    o.damages.append(damageNumber)
                                    damageNumber = damage(int(str(p.damage)[1]),p.owner,o,1,howFarDown)
                                    o.damages.append(damageNumber)
                                    noDuplicates = 1
